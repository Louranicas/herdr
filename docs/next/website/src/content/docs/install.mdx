...
A build on a channel that no published release manifest describes — a fork, or a local build stamped with its own channel — cannot resolve an update from a published manifest, because no published release ever carried that channel. `herdr update` refuses before fetching anything rather than replacing the build with an unrelated upstream binary. On a Linux or macOS direct install, set `HERDR_UPDATE_SOURCE` to a manifest URL for that build's own channel to update from it instead, or install a new build of it the way this one was installed — not through Homebrew, mise, or Nix, whose `herdr` packages are upstream's. The variable applies only to such builds, and it must be an `https://` URL. On stable and preview, `herdr update` refuses while it is set rather than quietly ignoring it: the manifest names both the binary and the checksum that binary is verified against, so honouring an environment-chosen one would let it choose the binary that replaces Herdr — unset it to update from the published manifest. It is refused for Homebrew, mise, and Nix installs too, because those are updated through the package manager rather than in place. It is refused on Windows as well, because the Windows update path installs through Herdr's install script rather than the asset the manifest names, so honouring it there would take the release notes from your source and the binary from upstream; update such a build the way it was installed. The configured update channel selects between the published manifests only, so a build that updates from `HERDR_UPDATE_SOURCE` fetches it whichever channel is set.

```bash
HERDR_UPDATE_SOURCE=https://example.com/my-fork.json herdr update
```
