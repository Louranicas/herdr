...
公開されているリリースマニフェストに記載のないチャネルのビルド（フォークや、独自のチャネルを刻印したローカルビルド）は、そのチャネルを持つリリースが公開されたことがないため、公開マニフェストからアップデートを解決できません。`herdr update` は、そのビルドを無関係な上流バイナリで置き換えるのではなく、何も取得する前に拒否します。代わりにそのビルド自身のチャネル向けマニフェスト URL を `HERDR_UPDATE_SOURCE` に設定するか、インストールした方法でアップデートしてください。この変数が効くのはそうしたビルドだけです。stable と preview のビルドは常に公開マニフェストからアップデートし、この変数を無視します。
