...
公開されているリリースマニフェストに記載のないチャネルのビルド（フォークや、独自のチャネルを刻印したローカルビルド）は、そのチャネルを持つリリースが公開されたことがないため、公開マニフェストからアップデートを解決できません。`herdr update` は、そのビルドを無関係な上流バイナリで置き換えるのではなく、何も取得する前に拒否します。Linux と macOS の直接インストールでは、そのビルド自身のチャネル向けマニフェスト URL を `HERDR_UPDATE_SOURCE` に設定するか、このビルドをインストールしたのと同じ方法で新しいビルドを入れてください。Homebrew、mise、Nix 経由は使えません。それらの `herdr` パッケージは上流のものだからです。この変数が効くのはそうしたビルドだけで、値は `https://` の URL でなければなりません。stable と preview では、設定されていると `herdr update` は黙って無視するのではなく拒否します。マニフェストはインストールするバイナリとその検証に使うチェックサムの両方を指定するため、環境が選んだマニフェストを尊重すると、Herdr を置き換えるバイナリをその環境に選ばせることになるからです。公開マニフェストからアップデートするには、この変数を解除してください。Homebrew、mise、Nix のインストールでも拒否されます。それらはインプレースではなくパッケージマネージャー経由でアップデートされるためです。Windows でも拒否されます。Windows のアップデート経路は、マニフェストが指定するアセットではなく Herdr のインストールスクリプト経由でインストールするため、そこでこの変数を尊重すると、リリースノートは指定した取得元から、バイナリは上流から取得されることになるからです。そうしたビルドは、インストールしたのと同じ方法でアップデートしてください。設定するアップデートチャンネルが選ぶのは公開マニフェストの間だけなので、`HERDR_UPDATE_SOURCE` からアップデートするビルドは、どちらのチャンネルが設定されていてもそこから取得します。
