...
レスポンスには `server_epoch` も含まれます。これは応答したサーバーのインカーネーション（サーバープロセスの世代）を識別するトークンです。サーバープロセスごとにメモリ上で一度だけ生成され、どこにも書き出されないため、保存済みのスナップショットが現在のプロセスと一致する値を持つことはありません。トークンを記録していたクライアントが後で異なる値を受け取った場合、サーバーが再起動またはハンドオフしており自身のキャッシュが古いと判断できます。スナップショットしか持たないクライアントがこの値を捏造することはできません。値を利用できない場合、このフィールドは空文字列になるのではなく欠落します。したがって、欠落を「空の epoch」と解釈してはいけません。同じフィールドはエージェントレコードにも現れます。このフィールドが検出する陳腐化のケースは、まさにエージェントがすでに存在しなくなっている可能性のあるケースだからです。
