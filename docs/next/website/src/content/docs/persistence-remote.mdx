...
For local builds or custom binaries, set `HERDR_REMOTE_BINARY` to a local file path before running remote attach. A client built on a channel that no published release manifest describes cannot resolve a remote binary automatically, so remote attach stops and names this variable instead of looking for a release that was never published.
