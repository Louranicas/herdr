...
在任何已发布发行清单都未描述的通道上构建的二进制文件(分叉,或标记了自定义通道的本地构建)无法从已发布的清单解析更新,因为从未有发布版本携带该通道。`herdr update` 会在获取任何内容之前拒绝,而不是用无关的上游二进制文件替换该构建。请将 `HERDR_UPDATE_SOURCE` 设为该构建自身通道的清单 URL 以改从它更新,或按安装时的方式更新。
