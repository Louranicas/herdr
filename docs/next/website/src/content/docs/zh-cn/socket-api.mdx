...
响应还包含 `server_epoch`,这是标识作出应答的服务端实例(incarnation)的令牌。它在每个服务端进程中只在内存里生成一次,且从不写入任何位置,因此已保存的快照不可能带有与当前进程匹配的值。记录过该令牌的客户端之后若看到不同的值,就能知道服务端已重启或完成交接、自身缓存已过期;而只持有快照的客户端无法伪造出这个值。当该值不可用时,此字段是缺失的,而不是空字符串,因此绝不能把缺失读作“空 epoch”。同一字段也出现在智能体记录上,因为它所检测的过期场景,正是智能体可能已经消失的场景。
