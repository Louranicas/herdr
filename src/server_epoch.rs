...
fn mint() -> Result<String, EntropyUnavailable> {
    let mut bytes = [0u8; EPOCH_BYTES];
    getrandom::fill(&mut bytes).map_err(EntropyUnavailable)?;
    Ok(bytes
        .iter()
...
mod tests {
    ...
    /// dependency would pass verbatim even if `mint` were reverted to deriving
